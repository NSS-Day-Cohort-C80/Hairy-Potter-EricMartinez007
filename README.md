...
1. Explain how you got the HTML, with the correct data, displayed in the browser?
   > First I needed to assign a function that would do two things: assign an empty string variable that could have HTML strings added to it and a for of loop that would go through every pottery object in the potteryToBeSold array and change it to an HTML string that included the pottery objects properties: id, shape, weight, and height. Additionally, the for of loop would take each HTML string produced and add it to the empty string variable. Once the for of loop went through each object in the array, the function would return the now filled string variable that contains all the HTML strings for each object it looked at. After the HTML strings for each pottery object were generated and assigned a variable, I needed to make a new function that would first assign a variable to the part of the DOM that I wanted to add my HTML strings to (targeting the id or class designation), then if that element was present in the DOM it would assign the target elements' innerHTML to the HTML strings that were generated earlier.  
2. In the **PotteryList** module, when you iterate your pottery, you need to show the evidence of what the **weight** property's value is for the 2nd piece of pottery. Use [Loom](https://www.loom.com/) to record your browser window with the developer tools open and show those values.
   > [Paste your video's public URL here](https://www.loom.com/share/97071dcbd62e4b0786aff59acd89dd46)
3. The **PotteryWheel** module has a single function named `makePottery`. Why doesn't that module have all of the other code in it?
   > We use separate modules for each function to keep the overall program more organized and readable. We want one main module that invokes all the functions in the program and a different module for each function. This allows us to test each function  without interrupting any of the other code. Additionally, the export/import command makes it easy to use the functions throughout the program. 
4. The pottery shop has learned that there is a set of customers that are willing to buy cracked pottery at a discounted price of $2.50. That means that the cracked pottery should now be displayed in the catalog. Explain the changes that this new business strategy would cause to your algorithm.
   > The only change this would cause would be in our toSellOrNotToSell function in the PotteryCatalog module. The toSellOrNotToSell function determines what gets added to our potteryToSell array and adds a price property to those objects getting added. Right now the cracked potteryObjects are not getting added to the array or getting a price property. The first step to fix this would be to edit my if statement on cracked objects from "if(potteryObject.cracked === true)" to "if(potteryObject.cracked === true){ potteryObject.price = 2.5}". I would then have to change my other if statements to else if statements to prevent them from overriding the cracked objects price property regardless of their weight. Finally to add everything to the potteryToSell array I would remove my if statement of "if(potteryObject.cracked === false){potteryToBeSold.push(potteryObject)}" and change it to "potteryToBeSold.push(potteryObject)" with no if argument. 
5. In the **Kiln** module, you have a `firePottery()` function. You need to demonstrate how to use the debugger to verify the values of the parameters for that function when your code runs. Use [Loom](https://www.loom.com/) to record your browser window with the developer tools open and show those values.
   > [Paste your video's public URL here](https://www.loom.com/share/8643027ee9284de2b9a409cecd8263fe)
